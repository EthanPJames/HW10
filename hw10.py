...
def conf_matrix(y_pred, y_true, num_class):
    ...
    M = np.zeros((num_class, num_class), dtype=float) #Create the empty matrix filled with zeroes
    for i in range(len(y_pred)):
        M[int(y_pred[i])][int(y_true[i])] += 1
    return M
...
def get_model_results(model_name, params, train_data, train_labels, test_data, test_labels, num_class):
    ...
    acc = metrics.accuracy_score(test_labels, y_predicted)
    # 5. Calculate the confusion matrix by using the completed the function above 
    conf_mat = conf_matrix(y_predicted, test_labels, num_class)
